...
Usage:
    ...
    -h WAITFORIT_HOST | --host=WAITFORIT_HOST       Host or IP under test
    -p WAITFORIT_PORT | --port=WAITFORIT_PORT       TCP port under test
                                Alternatively, you specify the host and port as host:port
    -s | --strict               Only execute subcommand if the test succeeds
    -q | --quiet                Don't output any status messages
    -t WAITFORIT_TIMEOUT | --timeout=WAITFORIT_TIMEOUT
