...
wait_for_wrapper()
{
    ...
    if [[ $QUIET -eq 1 ]]; then
        timeout $TIMEOUT $0 --quiet --child --host=$WAIT_HOST --port=$WAIT_PORT --timeout=$TIMEOUT &
    else
        timeout $TIMEOUT $0 --child --host=$WAIT_HOST --port=$WAIT_PORT --timeout=$TIMEOUT &
    fi
    PID=$!
